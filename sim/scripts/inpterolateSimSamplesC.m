...
for iWS=10.5 %unique(WS)'
    ...
    grid
    title(['Ship speed calculated from wind angle and main sail angle for Wind Speed=' num2str(iWS)]);
    ...
    zismu = imfilter(zismu, H, 'replicate');
    zismu(isnan(zismu)) = zi(isnan(zismu));
    surf(xi,yi,zismu);
    title(['Ship speed calculated from wind angle and main sail angle for Wind Speed=' num2str(iWS) ', SMOOTHED CUBIC interp']);
    xlabel('Wind Angle [deg]'), ylabel('Sail Angle [deg]'), zlabel('Ship speed [kn]')
    axis([0, 180, 0, 90, 0, 10]);
